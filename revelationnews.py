...
def call_api_bible_verse(chap_num, verse_num):
    ...
    headers = {"accept": "application/json", "api-key": f"{k}"}

    api_bible_response = requests.get(
        f"https://api.scripture.api.bible/v1/"
        f"bibles/{bible_ver}/verses/{ref}?{params}",
        headers=headers,
    )
    print("Called api.bible")
...
def call_news_api(topic):
    ...
    params = f"qInTitle={topic}&language=en&include-titles=true&sortBy=relevancy&pageSize=1&apiKey={k}"

    news_api_response = requests.get(f"https://newsapi.org/v2/everything?{params}")
    print("Called news.api")
    ...
    return topic, news_title, news_url, news_description
...
def new_search():
    ...
    print(f"Rev. {chap_num}:{verse_num}")
    # print(f'REVELATION\nChapter {chap_num}\nVerse {verse_num}')
    verse_str = call_api_bible_verse(chap_num, verse_num)
    cleaned_verse = clean_verse_up(verse_str)
    words = words_list_creation(verse_str)
    # print(words)
    topic = choose_topic(words)
    # print(topic.upper())
    bible_info = (chap_num, verse_num, cleaned_verse)
    news_info = call_news_api(topic)
    # print(bible_info)
    # print(news_info)
    return bible_info, news_info
